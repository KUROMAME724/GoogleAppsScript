...
function main() {
  //電車遅延情報をJSON形式で取得
  const delayInformationJson = JSON.parse(UrlFetchApp.fetch("https://rti-giken.jp/fhc/api/train_tetsudo/delay.json").getContentText());
  //路線名を指定
  const HANZOMON_LINE = "半蔵門線";
  //運営会社名を指定
  const TOKYO_METRO = "東京メトロ";
